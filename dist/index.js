        ...
        tools: [
            {
                ...
                description: "List all peripherals defined in an ARM CMSIS SVD file. Returns each peripheral's name, base address, and description. Use this to discover what hardware blocks are available in a microcontroller.",
                inputSchema: {
                    type: "object",
                    properties: {
                        svd_file: {
            ...
            },
            {
                ...
                description: "Get full details for a specific register within a peripheral, including address offset, size, access type, reset value, and all bit-fields with their positions. Use this when you need exact register layout for low-level firmware code.",
                inputSchema: {
                    type: "object",
                    properties: {
                        svd_file: {
            ...
            {
                name: "describe_field",
                description: "Describe a specific bit-field within a register: its bit offset, width, access type, and enumerated values if defined. Use this when you need to understand or set a specific control bit or flag.",
                inputSchema: {
                    type: "object",
                    properties: {
                        svd_file: {
            ...
            },
            {
                ...
                description: "Search across all peripherals and registers in an SVD file using a case-insensitive substring match. Returns up to 10 matches with peripheral name, register name, and description. Use this when you know part of a register name but not which peripheral it belongs to.",
                inputSchema: {
                    type: "object",
                    properties: {
                        svd_file: {
    ...
    try {
        switch (name) {
            case "list_peripherals": {
                ...
                if (!svd_file)
                    throw new Error('Required argument "svd_file" is missing');
                const device = (0, svd_parser_js_1.loadSvd)(svd_file);
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({ device: device.name, count: peripherals.length, peripherals }, null, 2),
                        },
                    ],
                };
            }
            case "lookup_register": {
                ...
                if (!svd_file)
                    throw new Error('Required argument "svd_file" is missing');
                if (!peripheralName)
                    throw new Error('Required argument "peripheral" is missing');
                if (!registerName)
                    throw new Error('Required argument "register" is missing');
                const device = (0, svd_parser_js_1.loadSvd)(svd_file);
                ...
                if (!register) {
                    throw new Error(`Register "${registerName}" not found in peripheral "${peripheral.name}". ` +
                ...
                const result = {
                    ...
                    resetValue: register.resetValue !== undefined
            ...
            case "describe_field": {
                const { svd_file, peripheral: peripheralName, register: registerName, field: fieldName, } = toolArgs;
                if (!svd_file)
                    throw new Error('Required argument "svd_file" is missing');
                if (!peripheralName)
                    throw new Error('Required argument "peripheral" is missing');
                if (!registerName)
                    throw new Error('Required argument "register" is missing');
                if (!fieldName)
                    throw new Error('Required argument "field" is missing');
                const device = (0, svd_parser_js_1.loadSvd)(svd_file);
                const peripheral = device.peripherals.find((p) => p.name.toLowerCase() === peripheralName.toLowerCase());
                if (!peripheral) {
                    throw new Error(`Peripheral "${peripheralName}" not found in ${device.name}`);
                ...
                if (!field) {
                    throw new Error(`Field "${fieldName}" not found in register "${register.name}". ` +
...
async function main() {
    ...
    if (restPort) {
        ...
        if (isNaN(port) || port < 1 || port > 65535) {
            ...
            (0, rest_js_1.startRestServer)(port);
        }
    }
